...
int main (int argc, char *argv[]) {
	int task_n, rank, vertex_s, vertex_n, edge_n, i, pre_f, terminate, finish, send, recv, m_count;
	vertex *vertices;
	int *work_q;
	int *pre_res;
	double t_comm, t_cal, t_sync, t, t_in;
	FILE *fh;
	...
	while(terminate == 0){
		...
		for(i=0; i<vertices[rank].adj_n; i++){
			if(vertices[rank].min_d == INT_MAX)send = INT_MAX;
			else send = vertices[rank].min_d+vertices[rank].w[i];
			//t = MPI_Wtime();
		...
		//t = MPI_Wtime();
		MPI_Reduce(&finish, &terminate, 1, MPI_INT, MPI_LOR , 0, MPI_COMM_WORLD);
	...
	//printf("# Message: %d @rank%d\n", m_count, rank);
	//printf("Comm  Time: %f @rank%d\n", t_comm, rank);
	//printf("Cal   Time: %f @rank%d\n", MPI_Wtime() - t_cal - t_sync - t_comm, rank);
	//printf("Cal   Time: %f @rank%d\n", MPI_Wtime() - t_cal - t_sync, rank);
	//printf("Sync  Time: %f @rank%d\n", t_sync, rank);
